...
const AnularApertura = () => {
  ...
  const handleRemoveApertura = async () => {
    setLoading(true);
  ...
  useEffect(() => {
    if (isSuccess && countdown > 0) {
      ...
      navigate("/caja-chica/aperturas"); // Ruta a la que quieres redirigir
    }
        ...
        {!isSuccess ? (
          ...
          <div className="p-8 flex flex-col items-center text-center gap-4">
            ...
            <p className="text-sm text-slate-500 mb-4">
              Estás a punto de anular esta apertura de caja chica. Esta acción
            ...
            {apertura && (
              <div className="bg-slate-50 border border-slate-100 rounded-lg p-4 w-full text-left mt-2 mb-4">
                ...
                <p className="text-sm text-slate-700">
                  ...
                  {apertura.trabajador?.nombre_trabajador || "N/A"}
                </p>
                <p className="text-sm text-slate-700">
                  <span className="font-semibold">Importe:</span> S/{" "}
                  {apertura.importe_apertura || "0.00"}
                </p>
              </div>
            )}
