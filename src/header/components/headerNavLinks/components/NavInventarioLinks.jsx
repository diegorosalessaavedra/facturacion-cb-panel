...
const NavInventarioLinks = ({
  openListModule,
  setOpenListModule,
  setIsMenuOpen, // Agregado para mantener consistencia con los demás módulos
}) => {
  const isOpen = openListModule === "inventario";
  ...
  return (
    <div className="w-[280px] bg-slate-900 flex flex-col border-t-[0.5px] border-slate-400">
      ...
      <div
        ...
        `}
        onClick={() => {
          setOpenListModule(isOpen ? "" : "inventario");
        }}
      >
        <div className="flex gap-4 items-center">
          {/* Al ser un engranaje (IoSettings), una rotación queda excelente como animación */}
          <IoSettings
            className={`text-xl transition-transform duration-500 ease-out ${
              isOpen ? "rotate-90 text-amber-500" : "text-slate-500"
            }`}
          />
      ...
      <div
        ...
        style={{
          // 100px es más que suficiente para un solo enlace
